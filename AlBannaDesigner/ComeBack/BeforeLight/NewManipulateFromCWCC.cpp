...
int main()
{
	...
	*/
	...
	GLfloat Ystep = 360.0f/10.0f, Zstep = 360.0f/5;
...
//	glBufferData(GL_ELEMENT_ARRAY_BUFFER, 12 * sizeof(GLint), TestSp, GL_DYNAMIC_DRAW);
...
//	mySphare->PrintIndices();
	...
	//	SphareShader.Use();
		...
		/*
		ÚäÏ ÇÓÊÎÏÇã ÇáÔÇÏÑ ÇáÓÇÈÞ ÇáÐí ÇáÛíäÇ ÝÅä ÇáÅßÓÇÁ ÇáËÇäí ÇáÎÇÕ ÈÇáãßÚÈÇÊ íÎÊÝí 
		æáÇßä ÚäÏåÇ íÚãá åÐ ÇáÔÇÏÑ ÇáËÇäí ÝäÊÍßã Ýí ÇáÇáæÇä 
		ÈíäãÇ Ýí ÍÇáÉ ÚÏã ÇÓÊÎÏÇãÉ æÇíÖÇ áã äÞæã ÈÚãá ÇßÓÇÁ ááßÑÉ ÇááÊí ääÔÃåÇ 
		ÝÛä ÇáÈÑäÇãÌ ÓíÓÊÎÏã ÇáæÇä ÇáÍæÇÝ 
		MyBorderColor 
		ÇááÊí æÖÚäÇåÇ ÚäÏ ÇÚÏÇÏ ÇáÇßÓÇÆíä ÇáÇæá æÇáËÇäí 
		ÓíÓÊÎÏãåÇ ÇáÈÑäÇãÌ ß áæä ÇáßÑÉ 
		ØÈÚÇ ÓíÞæã ÈÏãÌ Çáæäíä
	...
	//	glDrawElements(GL_TRIANGLES, mySphare->Indices.size(), GL_UNSIGNED_INT, 0);
		...
		int mmks = (((GLint)(4 * glfwGetTime()) % 10));
		if (mmks == 0)
			mmks = 1;
		generlConuter++;
		if (generlConuter >= 5000)
			generlConuter = 100;
		glDrawElements(GL_TRIANGLE_STRIP, mySphare->Indices.size() / (GLint)(generlConuter/100), GL_UNSIGNED_INT, 0);
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
		
		*/
		glPointSize(0.8f);
		glDrawElements(GL_LINES, mySphare->Indices.size(), GL_UNSIGNED_INT, 0);
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
